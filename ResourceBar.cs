...
namespace ImprovedFlightControls
{
	internal class ExampleResourceBar : UIState
	{
		...
		public override void OnInitialize() {
			...
			barFrame = new UIImage(ModContent.Request<Texture2D>("ImprovedFlightControls/ResourceBarFrame")); // Frame of our resource bar
		...
		public override void Draw(SpriteBatch spriteBatch) {
			...
			if (Main.LocalPlayer.wingTime==Main.LocalPlayer.wingTimeMax|| !ModContent.GetInstance<Config>().FlightTimer|| Main.gameMenu ||Main.LocalPlayer.dead)
				return;
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) {
			...
			// Calculate quotient
			float quotient = (float)Main.LocalPlayer.wingTime /Main.LocalPlayer.wingTimeMax; // Creating a quotient that represents the difference of your currentResource vs your maximumResource, resulting in a float of 0-1f.
		...
		public override void Update(GameTime gameTime) {
			...
			text.SetText(string.Format("{0:f" + 1 + "}s", Main.LocalPlayer.wingTime / 60f));
